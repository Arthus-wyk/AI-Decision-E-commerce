...
type CompareGroupsClientProps = {
  groups: HydratedCompareSession[];
  error?: string;
};

export function CompareGroupsClient({ groups, error }: CompareGroupsClientProps) {
  ...
  return (
    ...
    >
      <header style={{ marginBottom: "20px" }}>
        ...
        <p style={{ margin: "8px 0 0", color: "#445268" }}>View all existing product comparison groups.</p>
      </header>
              ...
              >
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", minWidth: 0, flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px" }}>
                    <strong style={{ fontSize: "16px", color: "#172033", wordBreak: "break-all" }}>{group.name}</strong>
                    <span
                      style={{
                        fontSize: "12px",
                        color: "#0b63d8",
                  ...
                  <p style={{ margin: 0, fontSize: "14px", color: "#445268" }}>User: {group.user_id || "Unknown"}</p>
                  <p style={{ margin: 0, fontSize: "13px", color: "#5d6d84" }}>Created at: {group.created_at || "N/A"}</p>
                  <p
                    style={{
                      margin: 0,
                      fontSize: "13px",
                  ...
                  >
                    {group.products.slice(0, 3).map((product) => product.name).join(" / ") || "No products"}
                  </p>
                </div>

                <div style={{ width: "148px", height: "90%", position: "relative", flexShrink: 0, alignSelf: "center" }}>
